  ...
  try {
    ...
    if (drawType === "multi") {
      await sql`
        ...
        WHERE id = ${validUserId} AND balance >= ${cost}
      `

      // Record balance transaction
      await sql`
        INSERT INTO balance_transactions (user_id, amount, type, description)
        VALUES (${validUserId}, ${cost}, 'draw_deduct', '5连抽消费')
      `
      ...
      for (const prize of prizes) {
        await sql`
          ...
          VALUES (${validUserId}, ${prize}, 'multi')
        `
      }
    } else {
      await sql`
      ...
      await sql`
        INSERT INTO balance_transactions (user_id, amount, type, description)
        VALUES (${validUserId}, ${cost}, 'draw_deduct', ${"单抽消费 - " + prizeName})
      `

      // Record in history
      await sql`
        INSERT INTO draw_history (user_id, prize_name, draw_type)
        VALUES (${validUserId}, ${prizeName}, 'single')
      `
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error("[v0] Failed to record draw:", error)
